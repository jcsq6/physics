...
world::world(double width, double height, double px_per_meter){
...
void world::update(double dt) {
	...
	std::pair<bool, vec2<double>> n;
	vec2<double> imp, relVel;
...
void world::draw(SDL_Renderer* renderer) {
	for (const auto& obj : objects) {
		//poly::draw_poly(renderer, obj.bounds.convert(s_converter, IS_GUARANTEED).convert(p_converter, IS_GUARANTEED));
	}
}